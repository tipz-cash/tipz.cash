...
export function TippingFlow({
  ...
  // Show pending tip notification as toast
  useEffect(() => {
    if (pendingTipNotification) {
      toast.success(
        `Delivering ${pendingTipNotification.amount} ${pendingTipNotification.tokenSymbol} to @${pendingTipNotification.creatorHandle}...`,
        {
          id: "pending-tip",
          duration: Infinity,
          onDismiss: dismissPendingTipNotification,
        }
      )
    } else {
      toast.dismiss("pending-tip")
    }
  }, [pendingTipNotification, dismissPendingTipNotification])

  // Show failed tip notification as toast
  useEffect(() => {
    if (failedTipNotification) {
      toast.warning(
        `Your tip to @${failedTipNotification.creatorHandle} was ${failedTipNotification.reason}`,
        {
          id: "failed-tip",
          duration: Infinity,
          onDismiss: dismissFailedTipNotification,
        }
      )
    } else {
      toast.dismiss("failed-tip")
    }
  }, [failedTipNotification, dismissFailedTipNotification])
  ...
  const handleTokenSelect = async (token: SupportedToken) => {
    ...
    const needsChainSwitch =
      token.chainId !== walletState.chainId && token.chainId !== 501 && walletState.chainId !== 501
    ...
    if (needsChainSwitch) {
      try {
        ...
        if (!switched) {
          ...
          setChainSwitchError(
            `Please switch to ${chainNames[token.chainId] || `Chain ${token.chainId}`} in your wallet`
          )
          return
        }
  ...
  const getContentKey = () => {
    if (
      ["signing", "processing", "delivering", "success", "error", "refunded"].includes(flowState)
    ) {
      return "transaction"
    }
            ...
            <div
              style={{
                ...
                paddingBottom: isMobile
                  ? `max(${tokens.space.lg}px, env(safe-area-inset-bottom))`
                  : tokens.space.lg,
